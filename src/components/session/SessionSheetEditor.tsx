  ...
  return (
    <section className="bcvb-tool-card coach-form-card">
      ...
      <fieldset className="coach-form-grid">
        <legend className="sr-only">Informations de la séance d'entraînement</legend>
        <div className="form-field">
          <label htmlFor="session-team">Équipe</label>
          <input id="session-team" value={session.team} onChange={(event) => update('team', event.target.value)} placeholder="Ex: U9 A" />
        </div>
        <div className="form-field">
          <label htmlFor="session-category">Catégorie</label>
          <input id="session-category" value={session.category} onChange={(event) => update('category', event.target.value)} placeholder="Ex: U9" />
        </div>
        <div className="form-field">
        ...
        <div className="form-field">
          ...
          <input id="session-coach" value={session.coach} onChange={(event) => update('coach', event.target.value)} placeholder="Nom du coach" />
        </div>
        <div className="form-field">
          <label htmlFor="session-duration">Durée</label>
          <input id="session-duration" value={session.totalDuration} onChange={(event) => update('totalDuration', event.target.value)} placeholder="Ex: 90 min" />
        </div>
        <div className="form-field">
          <label htmlFor="session-theme">Thème</label>
          <input id="session-theme" value={session.theme ?? ''} onChange={(event) => update('theme', event.target.value)} placeholder="Thème de séance" />
        </div>
        <div className="form-field">
          <label htmlFor="session-title">Titre séance</label>
          <input id="session-title" value={session.title} onChange={(event) => update('title', event.target.value)} placeholder="Titre descriptif" />
        </div>
        <div className="form-field">
          <label htmlFor="session-location">Lieu</label>
          <input id="session-location" value={session.location} onChange={(event) => update('location', event.target.value)} placeholder="Nom du lieu" />
        </div>
        <div className="form-field">
          <label htmlFor="session-format">Format</label>
          <select id="session-format" value={session.format} onChange={(event) => update('format', event.target.value as TrainingSession['format'])}>
        ...
        <div className="form-field">
          ...
          <textarea id="session-objective" value={session.mainObjective} onChange={(event) => update('mainObjective', event.target.value)} placeholder="Décrire l'objectif principal" />
        </div>
        <div className="form-field">
          <label htmlFor="session-secondary">Objectifs secondaires</label>
          <textarea id="session-secondary" value={session.secondaryObjectives} onChange={(event) => update('secondaryObjectives', event.target.value)} placeholder="Objectifs additionnels" />
        </div>
        <div className="form-field">
          <label htmlFor="session-material">Matériel</label>
          <textarea id="session-material" value={session.material} onChange={(event) => update('material', event.target.value)} placeholder="Liste du matériel nécessaire" />
        </div>
        <div className="form-field">
          <label htmlFor="session-players">Nombre de joueurs</label>
          <input id="session-players" value={session.playerCount} onChange={(event) => update('playerCount', event.target.value)} placeholder="Ex: 12" />
        </div>
        <div className="form-field">
          <label htmlFor="session-org">Organisation générale</label>
          <textarea id="session-org" value={session.generalOrganisation} onChange={(event) => update('generalOrganisation', event.target.value)} placeholder="Structure et organisation de la séance" />
        </div>
        <div className="form-field">
          <label htmlFor="session-step">Démarche pédagogique</label>
          <input id="session-step" value={session.pedagogicalStep} onChange={(event) => update('pedagogicalStep', event.target.value)} placeholder="Je découvre / Je m'exerce / Je retranscris / Je régule" />
        </div>
      </fieldset>
