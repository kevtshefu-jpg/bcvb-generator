...
import { downloadMarkdownDocument, downloadPdfDocument, downloadSourceDocument } from '../../../lib/downloadDocument'
import { useAuth } from '../../auth/context/AuthContext'
import { saveEditorialStudioState, defaultEditorialStudioState } from '../../../utils/editorialStudioStorage.js'
import {
  DOCUMENT_FAMILIES,
...
function uniq(values: Array<string | null | undefined>) {
  return Array.from(new Set(values.map(normalize).filter(Boolean))).sort((a, b) => a.localeCompare(b))
}

function optionUnion(values: string[], existing: string[]) {
  return Array.from(new Set([...values, ...existing].filter(Boolean))).sort((a, b) => a.localeCompare(b))
}

function getFamily(doc: LibraryDocumentRow) {
  return normalize(doc.family) || normalize(doc.subcategory) || normalize(doc.category) || normalize(doc.document_type) || 'Document BCVB'
}
...
function getSubCategory(doc: LibraryDocumentRow) {
  return normalize(doc.subCategory) || normalize(doc.sub_category) || normalize(doc.subcategory) || normalize(doc.category_code) || 'Non renseignée'
}
...
function getSportCategory(doc: LibraryDocumentRow) {
  return normalize(doc.sportCategory) || normalize(doc.sport_category) || normalize(doc.team_code) || normalize(doc.category_code) || 'Toutes catégories'
}
...
function getPublicationLevel(doc: LibraryDocumentRow) {
  return normalize(doc.publicationLevel) || normalize(doc.publication_level) || normalize(doc.level) || (doc.is_featured ? 'Référence BCVB' : 'Club')
}

function hasPdf(doc: LibraryDocumentRow) {
  return Boolean(doc.pdf_url || doc.pdf_storage_path || doc.pdfPath || doc.pdf_path || lower(doc.file_ext).includes('pdf') || lower(doc.document_type).includes('pdf'))
}

function hasSource(doc: LibraryDocumentRow) {
  return Boolean(doc.content?.trim() || doc.source_markdown?.trim() || doc.file_url || doc.storage_path || doc.sourcePath || doc.source_path || doc.sourceDownloadUrl || doc.source_download_url)
}
...
function hasVersions(doc: LibraryDocumentRow) {
  return Boolean((doc.versions_count || 0) > 1 || doc.parent_document_id || doc.parentDocumentId || doc.created_from_document_id || doc.createdFromDocumentId)
}
...
function loadLocalDraftDocuments(): LibraryDocumentRow[] {
  try {
    const drafts = JSON.parse(window.localStorage.getItem('bcvb-editorial-library-drafts') || '[]') as Array<{
      title: string
      family?: string
      content: string
      score?: number
...
export default function LibraryPage() {
  ...
  const libraryUser = { id: user?.id, role: profile?.role, category_id: profile?.category_id }
  const isAdmin = profile?.role === 'admin'

  useEffect(() => {
    const scrollY = Number(window.localStorage.getItem(SCROLL_STORAGE_KEY) || 0)
    if (scrollY > 0) window.requestAnimationFrame(() => window.scrollTo(0, scrollY))
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (active) setDocuments([...rows, ...loadLocalDraftDocuments()])
      } catch (err) {
        if (active) {
  ...
  const visibleDocuments = useMemo(
    () => documents.filter((doc) => {
      ...
      if (!isAdmin && (isArchived(doc) || isDeleted(doc))) return false
    ...
    [documents, filters.lifecycle, libraryUser.id, libraryUser.role, libraryUser.category_id]
  )
  ...
  const filteredDocuments = useMemo(() => {
    ...
    return visibleDocuments.filter((doc) => {
      ...
      const matchesSportCategory = filters.sportCategory === 'all' || getSportCategory(doc) === filters.sportCategory
      ...
      const matchesPublicationLevel = filters.publicationLevel === 'all' || getPublicationLevel(doc) === filters.publicationLevel
      const matchesTag = filters.tag === 'all' || (doc.tags || []).includes(filters.tag)

      return matchesSearch && matchesFamily && matchesCategory && matchesSubCategory && matchesTheme && matchesSportCategory && matchesAudience && matchesSeason && matchesStatus && matchesFileType && matchesPublicationLevel && matchesTag
    })
  }, [filters, visibleDocuments])

  const stats = useMemo(() => ({
    total: filteredDocuments.length,
    publishable: filteredDocuments.filter((doc) => /publiable|prêt|published|ready/i.test(`${doc.status} ${doc.quality_status}`)).length,
    toFix: filteredDocuments.filter((doc) => /corriger|draft|brouillon|review|à contrôler/i.test(`${doc.status} ${doc.quality_status}`)).length,
    ...
    recent: filteredDocuments.filter((doc) => Date.now() - new Date(doc.updated_at || doc.created_at).getTime() < 1000 * 60 * 60 * 24 * 30).length,
    archived: filteredDocuments.filter(isArchived).length,
  }), [filteredDocuments, libraryUser.role])
  ...
  async function handleDownloadPdf(doc: LibraryDocumentRow) {
    try {
      ...
      await downloadPdfDocument({ ...doc, content: doc.content || doc.source_markdown })
  ...
  async function handleDownloadSource(doc: LibraryDocumentRow) {
    try {
      setDownloadError(null)
      await downloadSourceDocument({ ...doc, content: doc.content || doc.source_markdown })
    } catch (err) {
  ...
  async function handleDownloadMarkdown(doc: LibraryDocumentRow) {
    try {
      setDownloadError(null)
      await downloadMarkdownDocument({ ...doc, content: doc.content || doc.source_markdown })
    } catch (err) {
  ...
  async function handleCopySource(doc: LibraryDocumentRow) {
    if (!doc.content?.trim() && !doc.source_markdown?.trim()) return
    await navigator.clipboard.writeText(doc.content || doc.source_markdown || '')
  ...
  async function handleArchive(doc: LibraryDocumentRow) {
    if (!isAdmin) return
    if (!window.confirm(`Archiver "${doc.title}" ? Le document sera retiré de la vue active sans être supprimé définitivement.`)) return
    ...
    try {
      ...
      setDocuments((current) => current.map((item) => item.id === doc.id ? { ...item, is_archived: true, status: 'archived', updated_at: new Date().toISOString() } : item))
  ...
  async function handleSoftDelete(doc: LibraryDocumentRow) {
    ...
    try {
      ...
      setDocuments((current) => current.map((item) => item.id === doc.id ? { ...item, is_deleted: true, delete_reason: reason.trim(), status: 'deleted', is_active: false, updated_at: new Date().toISOString() } : item))
  ...
  function renderSelect(label: string, key: keyof LibraryFilters, values: string[]) {
    return (
      <label className="library-filter">
        <span>{label}</span>
        <select value={filters[key]} onChange={(event) => patchFilters({ [key]: event.target.value } as Partial<LibraryFilters>)}>
  ...
  return (
    <main className="library-page bcvb-page">
      <section className="library-hero">
        <div>
          <p className="bcvb-eyebrow">Bibliothèque documentaire</p>
          <h1>Centre de ressources BCVB</h1>
          <p>Consulter, rechercher, ouvrir, prévisualiser, télécharger et transformer les documents du club selon les droits du profil connecté.</p>
        </div>
        <div className="library-view-toggle">
          <button type="button" className={filters.viewMode === 'grid' ? 'is-active' : ''} onClick={() => patchFilters({ viewMode: 'grid' })}>Grille</button>
          <button type="button" className={filters.viewMode === 'list' ? 'is-active' : ''} onClick={() => patchFilters({ viewMode: 'list' })}>Liste</button>
        </div>
      </section>

      <section className="library-stats">
        <article><span>Documents visibles</span><strong>{stats.total}</strong></article>
        ...
        <article><span>Sans PDF</span><strong>{stats.withoutPdf}</strong></article>
        <article><span>Transformables</span><strong>{stats.transformable}</strong></article>
        <article><span>Récents</span><strong>{stats.recent}</strong></article>
      </section>

      <section className="library-toolbar">
        <label className="library-search">
          <span>Recherche texte / sémantique simple</span>
          <input value={filters.search} onChange={(event) => patchFilters({ search: event.target.value })} placeholder="Titre, description, catégorie, tags, résumé, métadonnées..." />
        </label>
        <button type="button" onClick={() => setFilters(defaultFilters)}>Réinitialiser filtres</button>
      </section>
      ...
      <div className="library-layout">
        <aside className="library-filters">
          ...
          {isAdmin && renderSelect('Cycle de vie', 'lifecycle', ['active', 'archived', 'deleted', 'all'])}
        </aside>

        <section className="library-results">
          {safeLoading && <p>Chargement de l’espace BCVB...</p>}
          {hasTimedOut && PRESENTATION_MODE && (
            <div className="bcvb-demo-fallback">
              ...
              <p>Le chargement distant est temporairement indisponible. Les brouillons locaux restent affichables.</p>
            </div>
          )}
          {error && (
            <div className="bcvb-demo-fallback">
              ...
              <p>{PRESENTATION_MODE ? 'La bibliothèque reste disponible dès que la connexion aux données répond.' : error}</p>
            </div>
          )}
          {downloadError && (
            <div className="bcvb-demo-fallback">
          ...
          {actionMessage && <p className="library-action-message">{actionMessage}</p>}
              ...
              return (
                <article className="library-card" key={doc.id}>
                  <header>
                    ...
                    <span className={`library-card__status ${/corriger|draft|brouillon/i.test(getStatus(doc)) ? 'is-warning' : 'is-ready'}`}>{getStatus(doc)}</span>
                  </header>

                  <p className="library-card__description">{doc.description || doc.summary || 'Document sans description, affiché avec métadonnées minimales.'}</p>
                  ...
                  <div className="library-card__tags">
                    {(doc.tags || ['BCVB']).slice(0, 6).map((tag) => <span key={tag}>{tag}</span>)}
                  </div>

                  <div className="library-card__version">
                    <span>Version {getVersion(doc)}</span>
                    <span>Modifié le {new Date(doc.updated_at || doc.created_at).toLocaleDateString('fr-FR')}</span>
                    {hasVersions(doc) && <button type="button" onClick={() => setActionMessage('Versionnage détaillé à venir. Version actuelle conservée.')}>Voir versions</button>}
                  </div>

                  <div className="library-card__actions">
                    <button type="button" onClick={() => handleOpenDocument(doc)} disabled={openingId === doc.id}>{openingId === doc.id ? 'Ouverture...' : 'Ouvrir'}</button>
                    <button type="button" onClick={() => setPreviewDocument(doc)}>Prévisualiser</button>
                    <button type="button" onClick={() => handleDownloadPdf(doc)} disabled={!canPdf} title={!canPdf ? 'PDF à générer, mais aucune source exploitable.' : undefined}>
                      {pdfAvailable ? 'Télécharger PDF' : canGeneratePdf(doc) ? 'Générer PDF' : 'PDF à générer'}
                    </button>
                    <button type="button" onClick={() => handleDownloadSource(doc)} disabled={!sourceAvailable} title={!sourceAvailable ? 'Source indisponible.' : undefined}>
                      {sourceAvailable ? 'Télécharger source' : 'Source indisponible'}
                    </button>
                    <button type="button" onClick={() => handleDownloadMarkdown(doc)} disabled={!canGeneratePdf(doc)} title={!canGeneratePdf(doc) ? 'Source Markdown indisponible.' : undefined}>Télécharger source Markdown</button>
                    {transformAllowed && (
                      <button type="button" onClick={() => handleTransform(doc)} disabled={!sourceAvailable} title={!sourceAvailable ? 'Transformation impossible : source indisponible.' : undefined}>
                        Transformer en document BCVB
                      </button>
                    )}
                    {!transformAllowed && <span className="library-card__hint">Transformation réservée admin.</span>}
                    {isAdmin && (
                      <>
                        <button type="button" onClick={() => handleArchive(doc)} disabled={adminActionId === doc.id || isArchived(doc) || isDeleted(doc)}>
                          {isArchived(doc) ? 'Archivé' : 'Archiver'}
                        </button>
                        <button type="button" className="library-card__danger" onClick={() => handleSoftDelete(doc)} disabled={adminActionId === doc.id || isDeleted(doc)}>
